...
// ==================== 1. 太和殿：重檐庑殿顶·清代官式 ====================
// 四坡庑殿顶：正脊+前后坡+左右坡+檐口底板
function hipRoof(w, d, ridgeH, mRoofMat) {
  const g=new THREE.Group();
  const hw=w/2, hd=d/2, rh=0.16; // rh=正脊厚度
  // 正脊
...
function buildTaihedian() {
  ...
  const bB=pTop+2.7; // beamBase
  // 横梁(面阔方向·每榀一组七架梁/五架梁/三架梁)
...
// ==================== 2. 乾清宫：重檐庑殿顶·面阔9间进深5间·减柱造·下五踩上七踩斗拱 ====================
// 斗拱构件：斗(梯形方块带槽)
...
function buildQianqinggong() {
  ...
  // ======== L1: 柱网——面阔9间·减柱造·朱红柱+汉白玉柱础 ========
  const b1=new THREE.Group();
  // 檐柱(朱红·20根)
  ex.forEach(x=>ez.forEach(z=>{
    if(x===0&&(Math.abs(z)<0.3)) return; // 减柱造：明间前檐减金柱
    b1.add(qingPillar(2.5,false).setPos(x,pTop,z));
  }));
  // 金柱(深红·15根·更高)
  ix.forEach(x=>iz.forEach(z=>{
    if(x===0&&z===0) return; // 明间减柱
...
// ==================== 3. 赵州桥：极致细节·28道并列拱券·空腹小拱·蟠龙栏板·写实石材 ====================
function buildZhaozhouqiao() {
  ...
  const MT=()=>new THREE.MeshStandardMaterial({color:0x7a7a7a,roughness:0.25,metalness:0.7}); // 腰铁·铸铁
  ...
  for(let x=-deckLen/2;x<=deckLen/2;x+=0.7){
    const curve=1-(x/(deckLen/2))*(x/(deckLen/2)); // 拱形曲线0~1
...
// ==================== 4. 卢沟桥：11孔联拱·船型墩·501石狮·华表碑亭·极致细节 ====================
...
// ==================== 5. 四合院：标准一进院·1:1还原·蛮子门·前檐廊·抄手游廊·筒瓦·国赛标准 ====================
function buildSiheyuan() {
  ...
  const M={x:0,z:-2.8,w:4.5,d:2.0,wh:1.9,rh:0.6};   // 正房·面阔3间·带前檐廊
  const E={x:+2.8,z:+0.2,w:1.5,d:3.5,wh:1.6,rh:0.5}; // 东厢
  const WE={x:-2.8,z:+0.2,w:1.5,d:3.5,wh:1.6,rh:0.5};// 西厢
  const S={x:-0.5,z:+2.8,w:3.8,d:1.5,wh:1.4,rh:0.45};// 倒座·偏西留门位
  const pH={main:0.17,wing:0.12,south:0.1};           // 台基高·体现等级
...
// ==================== 6. 徽派三叠式马头墙·1:1比例·硬山顶·鹊尾翘角·瓦作完整·国赛标准 ====================
function buildHuipai() {
  ...
  // === 建筑尺寸(1单位≈3m·面阔5间≈15m/进深≈13.5m) ===
  const BW=5.2, BD=4.4, WH=2.8, RH=0.7;       // 宽/深/墙高/脊高
  const rTop=WH+gY+0.06;                        // 墙顶面Y
  const ridgeY=rTop+RH;                         // 正脊Y
  // 三叠阶[zMin,zMax,高出屋顶h](中心→外侧·每侧3阶对称)
  const tiers=[
    [-0.3,0.3,0.9],   // T0中心·最高(高出屋面0.9m)
    [-1.35,-0.3,0.55],[0.3,1.35,0.55], // T1中段(高出0.55m)
    [-2.2,-1.35,0.3],[1.35,2.2,0.3],   // T2外侧(高出0.3m)
  ];
  ...
  // ==================== L3: 三叠式马头墙·双侧对称·完整构件体系 ====================
  const b3=new THREE.Group();
  for(let sx=-1;sx<=1;sx+=2){// 左/右山墙
  ...
  // ======== 前墙正门(居中·底齐石勒脚·带框门楣) ========
  const doorH=2.4, doorW=1.2, doorBot=gY+0.42; // 门底=石勒脚顶面
  ...
  doorG.position.set(0,0,BD/2); b4.add(doorG); // 前立面居中
  ...
  function selectLayer(idx){ selIdx=idx;
    ...
    allClickable.forEach(m=>{
      if(m.userData.layerIdx===idx){
        m.userData._selOrig=m.material; // 保存原始材质
...
function setupSunmao(){
  ...
  const joinX=0; // 拼接面X(榫肩与梁端面接触位置)

  // ===== 右侧·榫眼件(母材·固定) =====
  // 梁左端面在joinX, 梁中心在joinX+beamL/2, 卯眼从joinX向右延伸tongL
  ...
  // ===== 左侧·榫头件(可拖拽·榫头向右突出·用Group包裹便于整体移动) =====
  const tenonRoot=new THREE.Group();
  // 主体(中心在0, 右端面在+bodyL/2)
  ...
  // 装配目标: 榫肩(tenonRoot内x=bodyL/2)对准joinX => tenonRoot.position.x = joinX - bodyL/2 = -bodyL/2
...
function setupDougong(){
  ...
  function addP(mesh,name,desc){
    ...
    mesh.userData={name,desc,removed:false,origY:mesh.position.y,origPos:mesh.position.clone()};
